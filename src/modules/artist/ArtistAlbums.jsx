...
const ArtistAlbums = () => {
    ...
    const fetchAlbums = async () => {
      ...
      try {
        const response = await fetch(
          `https://api.spotify.com/v1/artists/${artist.id}/albums?include_groups=appears_on%2Calbum&limit=50`,
          {
            headers: {
        ...
        setAppearsOn(
          data.items.filter((album) => album.album_group === "appears_on")
        );
      } catch (error) {
  ...
  return (
    <>
      {albums && albums.length === 0 && appearsOn && appearsOn.length === 0 && (
        <h1 className='no-data'> This artist does not have any albums yet. </h1>
      )}
